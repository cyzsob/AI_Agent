...
from app.rag.retriever import get_hybrid_retriever, tokenize_for_bm25
...
async def main():
    ...
    logger.info("========== 数据库中所有 chunk ==========")
    for i, row in enumerate(result):
        row_dict = dict(row)
        logger.info(f"\n[Chunk {i + 1}] id={row_dict['id'][:8]}...")
        logger.info(f"  内容: {row_dict['content_preview']}...")
        logger.info(f"  元数据: {row_dict['metadata']}")
    ...
    for query in queries:
        ...
        # 4c. 混合检索 + Rerank
        logger.info("--- [混合检索 (向量+BM25 → Rerank)] ---")
        try:
            ...
            if not hybrid_docs:
                ...
                for i, doc in enumerate(hybrid_docs):
                    ...
                    logger.info(f"  [{i + 1}] {score_str} {doc.page_content[:80]}...")
        except Exception as err:
            logger.warning(f"  混合检索失败: {err}")

        # 4d. 纯 RRF（关闭重排，对照验证重排增益）
        logger.info("--- [混合检索 (纯 RRF，无 Rerank)] ---")
        try:
            ...
            if not rrf_only_docs:
                ...
                for i, doc in enumerate(rrf_only_docs):
                    rrf_score = doc.metadata.get("rrfScore")
                    logger.info(f"  [{i + 1}] (rrf: {rrf_score:.4f}) {doc.page_content[:80]}...")
        except Exception as err:
            logger.warning(f"  纯 RRF 检索失败: {err}")
